...
static __maybe_unused int ytphy_read_mmd(struct mii_bus *bus, int phy_id, int device, u32 regnum)
{
	int ret;

	ret = mdiobus_write(bus, phy_id, 0x0d, device);
	if (ret < 0)
		return ret;
		
	ret = mdiobus_write(bus, phy_id, 0x0e, regnum);
	if (ret < 0)
		return ret;
		
	ret = mdiobus_write(bus, phy_id, 0x0d, 0x4000+device);
	if (ret < 0)
		return ret;
...
static __maybe_unused int ytphy_write_mmd(struct mii_bus *bus, int phy_id, int device, u32 regnum, u16 val)
{
	int ret;
	...
	return mdiobus_write(bus, phy_id, 0x0e, val);
}
...
static int yt8521_config_init(struct mii_bus *bus, int phy_id, bool tx, bool rx)
{
	int ret, val;

	/* access utp register */
	ytphy_write_ext(bus,phy_id,0xa000, 0);

	 /* disable auto sleep extreg */
	val = ytphy_read_ext(bus,phy_id, YT8521_EXTREG_SLEEP_CONTROL1);
	if (val < 0)
		return val;
		
	val &= (~BIT(YT8521_EN_SLEEP_SW_BIT));
	ret = ytphy_write_ext(bus, phy_id, YT8521_EXTREG_SLEEP_CONTROL1, val);
	if (ret < 0)
		return ret;

	/* output 125M mac_clk */
	ret = ytphy_write_ext(bus, phy_id, 0xa012,0x68 );
	if (ret < 0)
		return ret;
		
	val = ytphy_read_ext(bus,phy_id, 0x1f);
	if (val < 0)
		return val;

	val |= (3 << 1);
	ret = ytphy_write_ext(bus, phy_id, 0x1f, val);
	if (ret < 0)
		return ret;

	/*enable RXC clock when no wire plug */
	ret = ytphy_write_ext(bus, phy_id, 0xa000, 0);
	if (ret < 0)
		return ret;
	...
	/* disable rx delay */
	val = ytphy_read_ext(bus, phy_id,0xa001);
	if (!rx)
		val &= ~BIT(8);
	else
		val |= BIT(8);
	ret = ytphy_write_ext(bus, phy_id, 0xa001, val);
    if (ret < 0){
        return ret;
	}
	
	/* disable tx delay */
	val = ytphy_read_ext(bus, phy_id, 0xa003);
	
	if (!tx)
		val &= ~(0xf << 0); 
	ret = ytphy_write_ext(bus, phy_id, 0xa003, val);
        if (ret < 0)
                return ret;
...
static int yt8531_config_dealy(struct mii_bus *bus, int phy_id, bool tx, bool rx)
{
	int ret, val;
	...
	ret = ytphy_write_ext(bus, phy_id, 0xa012,0xd0);
	if (ret < 0)
		return ret;
...
static int dwmac_rk_phy_config_init(struct stmmac_priv *priv)
{
	...
	case PHY_ID_YT8521:
		printk("hcq %s\n",__func__);
		yt8521_config_init(priv->mii, 0, false, false);
		break;
	case PHY_ID_YT8531:
		printk("hcq %s\n",__func__);
		yt8531_config_dealy(priv->mii, 0, false, false);
		break;
	default:
		break;
	}
...
static ssize_t
mii_reg_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	for (index = 0; index < 32; index++)
		sprintf(buf, "%s%2d: 0x%x\n", buf, index,
			mdiobus_read(priv->mii,0, index));

	return strlen(buf);
}

static ssize_t
mii_reg_store(struct device *dev,
		    ...
		    const char *buf, size_t count)
{
	...
	*data = 0;
	...
	mdiobus_write(priv->mii, 0, index, val);
...
static DEVICE_ATTR_RW(mii_reg);

static int g_ext_reg;

static ssize_t yt_ext_reg_show(struct device *dev,
				    struct device_attribute *attr,
				    char *buf)
{
	struct net_device *ndev = dev_get_drvdata(dev);
	...
	return sprintf(buf, "read extreg: 0x%x, 0x%x\n",
		       g_ext_reg, ytphy_read_ext(priv->mii,0,g_ext_reg));

}

static ssize_t yt_ext_reg_store(struct device *dev,
		    struct device_attribute *attr,
		    const char *buf, size_t count)
{
	struct net_device *ndev = dev_get_drvdata(dev);
	...
	if (!data){
		if (kstrtoint(tmp, 0, &index))
		    pr_info("please input a Register number at least\n"); 
		goto out1;
	}
	*data = 0;
	data++;
	...
	if (kstrtoint(data, 0, &val) || val == 0)
		goto out;
	
	g_ext_reg = index;
	  
	pr_info("Set extreg 0x%x to 0x%x\n", index, (int)val);
	
	ytphy_write_ext(priv->mii, 0, index, val);
...
out1:
	g_ext_reg = index;
	pr_info("Set extreg addr 0x%x\n", g_ext_reg);
	return count;
}
static DEVICE_ATTR_RW(yt_ext_reg);


static int g_mmd_dev_addr;
static int g_mmd_reg_addr;
static __maybe_unused ssize_t yt_mmd_reg_show(struct device *dev,
				    struct device_attribute *attr,
				    char *buf)
{
	struct net_device *ndev = dev_get_drvdata(dev);
	...
	return sprintf(buf, "read mmd_reg: 0x%x, 0x%x, 0x%x\n",
		       g_mmd_dev_addr, g_mmd_reg_addr, ytphy_read_mmd(priv->mii,0,g_mmd_dev_addr,g_mmd_reg_addr));

}

static __maybe_unused ssize_t yt_mmd_reg_store(struct device *dev,
		    struct device_attribute *attr,
		    const char *buf, size_t count)
{
	printk("no implement\n");
...
static DEVICE_ATTR_RW(yt_mmd_reg);
...
int dwmac_rk_create_loopback_sysfs(struct device *device)
{
	...
	ret = device_create_file(device, &dev_attr_mii_reg);
	if (ret)
		goto remove_mii_reg;
	
	ret = device_create_file(device, &dev_attr_yt_ext_reg);
	if (ret)
		goto remove_yt_ext_reg;

	ret = device_create_file(device, &dev_attr_yt_mmd_reg);
	if (ret)
		goto remove_yt_mmd_reg;
...
remove_mii_reg:
	device_remove_file(device, &dev_attr_mii_reg);
	
remove_yt_ext_reg:
	device_remove_file(device, &dev_attr_yt_ext_reg);
	
remove_yt_mmd_reg:
	device_remove_file(device, &dev_attr_yt_mmd_reg);
